...
public class PrimeOperations {
	...
	public ArrayList<Boolean> boolPrimes = new ArrayList<Boolean>();
	...
	{
		/*
		 * We will be using an algorithm which I've learned from MATH 470 - COMM AND CRYPTOGRAPHY
		 * This is known as The Quadratic Sieve (Sieve of Eratosthenes) to generate a list of primes from a BigInteger
		 * The algorithm goes as follows:
		 * 1. List integers from 1 to n and set i = 1, the smallest prime
		 * 2. Cross out numbers in increments of i*i for every number that has not been crossed out (make them false)
		 * 3. Continue to do so that it does not exceed n
		 * 4. What is not crossed is your list of primes (what is true)
		 */
		for(int i = 0; i <= count; i++) {
			boolPrimes.add(true);	//Creating list of true variables
		}
		//
		for (int j = 2; j*j < boolPrimes.size(); j++) {	//Since we know 1 is prime, we can start with j=2
			if(boolPrimes.get(j) == true) {
				for(int i = j*j; i < boolPrimes.size(); i += j) {
					boolPrimes.set(i, false);			//False for every j*j numbers
				}
			}
		}
		for (int k = 2; k < boolPrimes.size(); k++) {	//Add all primes to the list
			if(boolPrimes.get(k) == true) {
